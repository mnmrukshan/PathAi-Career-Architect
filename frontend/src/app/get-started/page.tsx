...
export default function GetStarted() {
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden bg-[#0A0A0E] text-white">
      {/* Background Decor */}
      ...
      <nav className="relative z-10 flex items-center justify-between px-8 py-6 border-b border-white/5 bg-black/20 backdrop-blur-md">
        <Link href="/" className="flex items-center gap-2 cursor-pointer group hover:opacity-80 transition-opacity">
          <div className="p-2 bg-gradient-to-tr from-violet-600 to-pink-500 rounded-lg group-hover:scale-105 transition-transform">
            <BrainCircuit className="w-6 h-6 text-white" />
          </div>
          <span className="text-xl font-bold tracking-tight group-hover:text-violet-400 transition-colors">PathAI</span>
        </Link>
        <div className="flex items-center gap-4 text-sm font-medium text-gray-400">
          ...
          <Link href="/login" className="px-5 py-2 rounded-full border border-white/10 hover:border-violet-500/50 hover:bg-violet-500/10 transition-all text-white">
            Sign In
          </Link>
        </div>
      </nav>
